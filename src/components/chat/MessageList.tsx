...
function getToolDisplayMessage(toolName: string, args: Record<string, any>, isComplete: boolean): string {
  if (toolName === "str_replace_editor") {
    ...
    switch (args?.command) {
      ...
      case "insert":
        return isComplete ? `Inserted into ${fileName}` : `Inserting into ${fileName}...`;
      case "view":
  ...
  if (toolName === "file_manager") {
    ...
    switch (args?.command) {
      case "rename":
        const newName = args?.new_path?.split("/").pop() || "file";
        return isComplete ? `Renamed ${fileName} → ${newName}` : `Renaming ${fileName}...`;
      case "delete":
        return isComplete ? `Deleted ${fileName}` : `Deleting ${fileName}...`;
      default:
        return isComplete ? `Managed ${fileName}` : `Managing ${fileName}...`;
    }
  }
  return toolName;
}
                              ...
                              <div key={partIndex} className="inline-flex items-center gap-2 mt-2 px-3 py-1.5 bg-neutral-50 rounded-lg text-xs border border-neutral-200">
                                {tool.state === "result" && tool.result ? (
                                  <>
                                    ...
                                    <span className="text-neutral-700">{getToolDisplayMessage(tool.toolName, tool.args, true)}</span>
                                  </>
                                ) : (
                                  <>
                                    <Loader2 className="w-3 h-3 animate-spin text-blue-600" />
                                    <span className="text-neutral-700">{getToolDisplayMessage(tool.toolName, tool.args, false)}</span>
                                  </>
                                )}
                              </div>
                            );
